...
public class Hotel extends SerializableEntity implements Comparable<Hotel>{
     ...
     */
    private Float computeWeight(Review review) throws DateFormatException{
        ...
        try {
            ...
            throw new DateFormatException("Il formato della data della review "+review.getHotel()+review.getUserId()+" non è corretto "+e.getMessage());
        }
    }

    private Float[] reviewWeight(List<Review> reviews) throws DateFormatException{
        ...
        Float sumWeight = Arrays.stream(weight).reduce(0.0f, Float::sum);

        //Normalizzo per opreservare la scala dei punteggi
        for (int i = 0; i < reviews.size(); i++) {
    ...
    private void updateRatings()throws DateFormatException{
        float rate = 0;
        float cleaning = 0;
        float position = 0;
        float services = 0;
        ...
        this.rate = Math.round(rate/this.reviews.size() * 10.0f) / 10.0f;

        cleaning = Math.round(cleaning/this.reviews.size() * 10.0f) / 10.0f;
        position = Math.round(position/this.reviews.size() * 10.0f) / 10.0f;
        services = Math.round(services/this.reviews.size() * 10.0f) / 10.0f;
        quality = Math.round(quality/this.reviews.size() * 10.0f) / 10.0f;
